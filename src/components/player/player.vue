      ...
      <div class="mini-player" @click="open" v-show="!fullScreen">
        ...
        <div class="control">
          <progress-circle :radius="radius" :percent="percent">
            <i :class="nimiIcon" class="icon-mini" @click.stop="togglePlaying"></i>
          </progress-circle>
        </div>
...
<script>
  ...
  import {getVkey} from 'common/js/getVkey'
  ...
  export default {
    ...
    created() {
      this.touch = {} // 用来关联touchstart touchmove的相关数据
    },
    watch: {
      currentSong(newSong, oldSong) {
        ...
        this.getLyric()// 获取歌词
        getVkey(this.currentSong.mid, this.currentSong.strMediaMid).then((url) => {
          if (url === 'getVkeyErr') {
            console.log('获取资源失败')
            return
          }
    ...
    methods: {
      ...
      format(interval) {
        ...
        const second = ('00' + interval % 60).slice(-2)
